...
### 2. Prepare Manifests
Before spawning agents, set up the manifest directory:
```bash
mkdir -p _context/manifests/$(date -u +%Y-%m-%d)
```
Then read the manifest-writer protocol:
```bash
...
Inject the manifest-writer protocol into each agent's prompt:
...
Task:
  ...
  prompt: |
    ...
    ---
    MANIFEST PROTOCOL:
    Write your findings to: _context/manifests/{date}/stream-{x}.jsonl
